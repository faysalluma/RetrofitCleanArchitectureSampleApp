...
class ConnectivityManagerUtils(context: Context, private val externalScope: CoroutineScope) {
    ...
    private val _connectionFlow = callbackFlow {
        val networkCallback = object : ConnectivityManager.NetworkCallback() {
            ...
            override fun onCapabilitiesChanged(network : Network, networkCapabilities : NetworkCapabilities) {
                val isInternetCapability = networkCapabilities.hasCapability(NetworkCapabilities.NET_CAPABILITY_INTERNET)
                val isValidated = networkCapabilities.hasCapability(NetworkCapabilities.NET_CAPABILITY_VALIDATED)
                if (isInternetCapability && isValidated && hasCorrectTransport(networkCapabilities)) {
                    trySend(true)
                }
            }
        }
        ...
        get() {
            ...
            return if (activeNetwork == null) {
                ...
                (netCapabilities != null
                        ...
                        && hasCorrectTransport(netCapabilities))
            }
        }

    private fun hasCorrectTransport(networkCapabilities: NetworkCapabilities): Boolean {
